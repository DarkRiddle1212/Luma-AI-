"""
Unit tests for ProviderLLMClient integration.

Tests cover:
- Delegation to provider.generate() with correct prompt and options
- Provider response dict conversion to LLMResponse (all fields preserved)
- Transient ProviderError triggers retry up to max_retries times
- Non-transient ProviderError raises LLMClientError immediately (no retry)
- After exhausting retries, raises LLMClientError with attempt count
- Retry events are logged via StructuredLogger
- Exponential backoff: sleep(2^0), sleep(2^1), sleep(2^2) between retries
...
# ---------------------------------------------------------------------------
# Test: response dict to LLMResponse conversion
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Test: non-transient errors raise immediately
# ---------------------------------------------------------------------------

class TestNonTransientErrorImmediate:
